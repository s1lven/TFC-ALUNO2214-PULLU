...
import { DollarCircleFreeIcons, LoadingFreeIcons, SparklesFreeIcons } from '@hugeicons/core-free-icons';
...
export default function CollectionImport({
  ...
  const applyPriceAdjustments = async () => {
    ...
    try {
      ...
      setAdjustPercentInput('');
      setMaxPriceInput('');
      setRoundMode('none');
      setCustomEndingInput('0.95');
      setRatePreview(null);
    } catch (e) {
  ...
  const importCollectionToShopify = async () => {
    ...
    try {
      ...
      if (collectionMode === 'new') {
        ...
        devLog('🆕 Creating new collection with name:', newCollectionName);
        ...
        devLog('✅ Collection created:', collectionResult.collection);
      ...
      for (let i = 0; i < productsToImport.length; i += batchSize) {
        ...
        if (selectedLanguage) {
          devLog(`🌍 Translating products ${i + 1}-${i + batch.length}...`);
          setImportProgress({ 
            current: i, 
          ...
          devLog(`✅ Translated ${translatedBatch.length} products`);
        }

        // Step 2b: Import translated/original products in batch concurrently
        devLog(`📦 Importing products ${i + 1}-${i + translatedBatch.length}...`);
          ...
          while (retries < maxRetries) {
            try {
              ...
              if (productResponse.status === 429) {
                ...
                devLog(`Rate limited on ${productTyped.title}, waiting ${waitTime}ms`);
              ...
              if (productResult.success) {
                devLog(`✅ Product ${globalIndex + 1}: "${productTyped.title}" imported, ID: ${productResult.product?.id}`);
                return { 
                  success: true, 
        ...
        batchResults.forEach(result => {
          if (result.success) {
            ...
            if (result.productId && result.originalIndex !== undefined) {
              ...
              devLog(`📝 Tracked product at index ${result.originalIndex}: ${productGid}`);
            } else {
        ...
        devLog(`📊 Batch complete: ${imported} imported, ${failed} failed. Total tracked: ${productIdMap.size}`);
      ...
      devLog(`🎯 All products imported. ProductIdMap size: ${productIdMap.size}`);
      ...
      if (productIdMap.size > 0) {
        try {
          ...
          devLog('🔄 Reordering', orderedProductIds.length, 'products to preserve original order...');
          devLog('Product IDs:', orderedProductIds);
          ...
          if (reorderResult.success) {
            devLog('✅ Products successfully reordered in collection!');
          } else {
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          ...
          >
            <DropdownMenuTrigger asChild>
              <Button
                size="sm"
                className="bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border-0 h-8 px-3 text-xs font-medium"
              >
                <HugeiconsIcon icon={DollarCircleFreeIcons} size={16} className="mr-2" />
                Adjust prices
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent
                ...
                >
                  {priceAdjustLoading ? (
                    <span className="flex items-center justify-center gap-2">
                      <HugeiconsIcon icon={LoadingFreeIcons} size={16} className="animate-spin" />
                      Applying…
                    </span>
                  ) : (
            ...
            <DropdownMenuContent className="bg-white border-gray-200 w-[380px] p-0 shadow-xl">
              ...
              <div className="bg-purple-50 border-b border-purple-100 px-4 py-2">
                <div className="flex items-start gap-2">
                  <HugeiconsIcon icon={SparklesFreeIcons} size={14} className="mt-0.5 flex-shrink-0 text-purple-600" />
                  <div>
              ...
              <div className="p-3">
                ...
                <Input
                  ...
                  className="bg-gray-50 border-gray-300 text-gray-900 h-8 text-xs mb-3"
                />
                <div className="max-h-[240px] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent hover:scrollbar-thumb-gray-400 border border-gray-200 rounded-lg">
                  {filteredLanguages.map((lang) => (
                    <button
                      ...
                      className={`w-full text-left px-3 py-1 cursor-pointer text-sm transition-all flex items-center justify-between ${
                ...
                {selectedLanguage && (
                  <div className="mt-2 pt-2 border-t border-gray-200">
                    <button
                      ...
                      className="w-full text-xs text-red-600 hover:text-red-700 hover:bg-red-50 font-medium py-1.5 rounded-md transition-colors"
                    >
                      Clear Selection
                    </button>
                  </div>
              ...
              <div className="flex-1 max-w-xl relative">
                ...
                >
                  {collectionMode === 'new' ? (
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <button className="w-full h-8 bg-white border border-gray-300 text-gray-900 rounded-md px-3 text-sm hover:border-gray-400 text-left flex items-center justify-between">
                      ...
                      <DropdownMenuContent className="bg-white border-gray-200 max-h-[300px] overflow-y-auto w-[var(--radix-dropdown-menu-trigger-width)]">
                        {collections.length === 0 ? (
                          <div className="px-3 py-2.5 text-sm text-gray-500 text-center">No collections found</div>
                        ) : (
                          collections.map((collection: { id: string; title: string; [key: string]: unknown }) => (
                            <div
                              ...
                              className="text-gray-900 cursor-pointer flex items-center gap-2.5 px-3 py-1 rounded-sm hover:bg-gray-50 transition-colors text-sm"
                          >
                            <div
          ...
          <div className="mb-4 pb-4 border-b border-gray-200">
            <label className="flex items-center gap-3 cursor-pointer group">
              <div className="relative flex items-center justify-center">
                ...
                />
                <div
                  className={`w-4 h-4 border-2 rounded transition-all ${selectAll ? 'bg-brand border-brand' : 'bg-white border-gray-300'}`}
                >
                  {selectAll && (
                    <svg className="w-full h-full text-gray-800" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
              ...
              {(collectionData.products as CollectionProduct[]).map((product, index: number) => (
                ...
                >
                  <div className="relative flex items-center justify-center flex-shrink-0">
                    ...
                    />
                    <div
                      className={`w-4 h-4 border-2 rounded transition-all ${selectedProducts.includes(index) ? 'bg-brand border-brand' : 'bg-white border-gray-300'}`}
                    >
                      {selectedProducts.includes(index) && (
                        <svg className="w-full h-full text-gray-800" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
