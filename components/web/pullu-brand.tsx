...
export function PulluMark({ size = 28, className }: MarkProps) {
  return (
    <svg
      width={size}
      height={size}
...
/** Mark + wordmark row (e.g. header). Wraps in Link when `href` is set. */
export function PulluWordmark({
  markSize = 28,
  wordmarkClassName = 'text-2xl font-bold text-gray-900 font-league-spartan',
  ...
  const inner = (
    <>
      <PulluMark size={markSize} />
      <span className={wordmarkClassName}>{children}</span>
    </>
  )
  if (href) {
    return (
